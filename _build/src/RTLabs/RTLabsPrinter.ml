...
*)
  ...
  |RTLabs.St_addi (dstr, src, i, lbl) -> Printf.sprintf "addi %s, %s, %s --> %s"
    (print_reg dstr)
    (print_reg src)
    (string_of_int i)
    lbl
  
|RTLabs.St_lw (_, dstr, i, addr, lbl) -> Printf.sprintf "lw %s, %s(%s) --> %s"
    (print_reg dstr)
    (string_of_int i)
    (print_reg addr)
    lbl

  |RTLabs.St_sw (_, srcr, i, addr, lbl) -> Printf.sprintf "sw %s, %s(%s) --> %s"
    (print_reg srcr)
    (string_of_int i)
    (print_reg addr)
    lbl
