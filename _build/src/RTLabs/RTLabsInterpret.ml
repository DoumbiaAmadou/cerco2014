...
*)
	...
	ReturnState (sfrs, v, mem, trace)
      ...
      | RTLabs.St_addi (dstr, srcr, i,  lbl)  -> 
	let v = Val.add (get_value lenv srcr) (Val.of_int i) in
       	assign_state sfrs graph sp  lbl lenv mem trace dstr v
	
      | RTLabs.St_lw (q,dstr,i , addr, lbl) -> 	
	let addr = address_of_value (Val.add (get_value lenv addr ) (Val.of_int i )) in
	let v = Mem.loadq mem q addr in
	assign_state sfrs graph sp lbl lenv mem trace dstr (v)  
	  
      | RTLabs.St_sw (q, srcr, i, addr, lbl) -> 
	let naddr = address_of_value (Val.add (get_value lenv addr) (Val.of_int i)) in
	let v = get_value lenv srcr in
	let nmem = Mem.storeq mem q naddr v in
	State (sfrs, graph, sp, lbl, lenv, nmem, trace)

      | RTLabs.St_cmp_zero (cond, srcr, lblcmp, lbl) ->
	begin
	  match cond with 
	  |AST.Cmp_eq -> if(Val.is_true (Val.cmp_eq (get_value lenv srcr) (Val.of_int 0))) then
	      State (sfrs, graph, sp, lblcmp, lenv, mem, trace)
	    else 
	      State (sfrs, graph, sp, lbl, lenv, mem, trace)
		
	  |AST.Cmp_ne -> if(Val.is_true (Val.cmp_ne (get_value lenv srcr) (Val.of_int 0))) then
	      State (sfrs, graph, sp, lblcmp, lenv, mem, trace)
	    else 
	      State (sfrs, graph, sp, lbl, lenv, mem, trace)
		
	  |AST.Cmp_gt -> if(Val.is_true (Val.cmp_gt (get_value lenv srcr) (Val.of_int 0))) then
	      State (sfrs, graph, sp, lblcmp, lenv, mem, trace)
	    else 
	      State (sfrs, graph, sp, lbl, lenv, mem, trace)
		
          |AST.Cmp_ge -> if(Val.is_true (Val.cmp_ge (get_value lenv srcr) (Val.of_int 0))) then
	      State (sfrs, graph, sp, lblcmp, lenv, mem, trace)
	    else 
	      State (sfrs, graph, sp, lbl, lenv, mem, trace)
		
          |AST.Cmp_lt -> if(Val.is_true (Val.cmp_lt (get_value lenv srcr) (Val.of_int 0))) then
	      State (sfrs, graph, sp, lblcmp, lenv, mem, trace)
	    else 
	      State (sfrs, graph, sp, lbl, lenv, mem, trace)
		
          |AST.Cmp_le -> if(Val.is_true (Val.cmp_le (get_value lenv srcr) (Val.of_int 0))) then
	      State (sfrs, graph, sp, lblcmp, lenv, mem, trace)
	    else 
	      State (sfrs, graph, sp, lbl, lenv, mem, trace)
	end
      ...
      | RTLabs.St_cmp (cond, srcr1, srcr2, lblcmp, lbl) ->
	begin
	  match cond with 
	  |AST.Cmp_eq -> if(Val.is_true (Val.cmp_eq (get_value lenv srcr1) (get_value lenv srcr2))) then
	      State (sfrs, graph, sp, lblcmp, lenv, mem, trace)
	    else 
	      State (sfrs, graph, sp, lbl, lenv, mem, trace)
		
	  |AST.Cmp_ne -> if(Val.is_true (Val.cmp_ne (get_value lenv srcr1) (get_value lenv srcr2))) then
	      State (sfrs, graph, sp, lblcmp, lenv, mem, trace)
	    else 
	      State (sfrs, graph, sp, lbl, lenv, mem, trace)
		
	  |AST.Cmp_gt -> if(Val.is_true (Val.cmp_gt (get_value lenv srcr1) (get_value lenv srcr2))) then
	      State (sfrs, graph, sp, lblcmp, lenv, mem, trace)
	    else 
	      State (sfrs, graph, sp, lbl, lenv, mem, trace)
		
          |AST.Cmp_ge -> if(Val.is_true (Val.cmp_ge (get_value lenv srcr1) (get_value lenv srcr2))) then
	      State (sfrs, graph, sp, lblcmp, lenv, mem, trace)
	    else 
	      State (sfrs, graph, sp, lbl, lenv, mem, trace)
		
          |AST.Cmp_lt -> if(Val.is_true (Val.cmp_lt (get_value lenv srcr1) (get_value lenv srcr2))) then
	      State (sfrs, graph, sp, lblcmp, lenv, mem, trace)
	    else 
	      State (sfrs, graph, sp, lbl, lenv, mem, trace)
		
          |AST.Cmp_le -> if(Val.is_true (Val.cmp_le (get_value lenv srcr1) (get_value lenv srcr2))) then
	      State (sfrs, graph, sp, lblcmp, lenv, mem, trace)
	    else 
	      State (sfrs, graph, sp, lbl, lenv, mem, trace)
	end
